...
contract ConstantProductAMM {
    ...
    constructor(address _token0, address _token1, address _apWallet) {
    ...
    function _internalSwap(int256 amount) internal {
        if (amount < 0) {
            uint256 abs_amount = uint256(-amount);
    ...
    function balancePool(uint256 oldPrice, uint256 newPrice) internal {
        ...
        uint256 balancedReserve0 = sqrt(K/newPrice);
        uint256 balancedReserve1 = balancedReserve0 * newPrice;
        ...
        // move excess tokens to wallet
        if (reserve1 > balancedReserve1) {
            uint256 amountToTransfer = reserve1-balancedReserve1;
            require(reserve1 >= amountToTransfer, "Not enough liquidity for AP transfer");
            token1.transfer(apWallet, amountToTransfer/SCALE);
            reserve1 -= amountToTransfer;
        }
    }
